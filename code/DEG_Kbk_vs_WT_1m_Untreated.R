...
# WT vs Kbk 1m Untreated DEG and heatmaps --------------------------------------
...
# microglia number/group --------------------------------------------------



MG_num_per_group<-data.frame(table(MG@meta.data$group))

names(MG_num_per_group)<-c("group","cell_num")


write.csv(MG_num_per_group, file="results/Microglia numer per group.csv")
...
MG_Kbk_WT_1m_Un<-subset(MG,Age=="1m" & Treatment =="Un" & Genotype %in% c("WT","Kbk"))

#reassign to another variable: seurat
seurat<-MG_Kbk_WT_1m_Un
...
write.csv(metadata,file="results/metadata_Kbk_vs_WT_1m_Untreated.csv")
...
saveRDS(dds,"data/dds_obj_Kbk_vs_WT_1m_Un.rds")
...
WT.1m.Un<-levels(factor(metadata$group))[2]#"WT.1m.Un"
Kbk.1m.Un<-levels(factor(metadata$group))[1]#"Kbk.1m.Un"


contrast <- c("group", Kbk.1m.Un, WT.1m.Un)
...
Kbk_vs_WT_1m_Un_pvalue<-res_tbl %>% filter(pvalue<0.05)


#setwd("C:/Users/kppra/OneDrive/Desktop/scRNAseq_Seurat5/results_seurat5")
write.csv(Kbk_vs_WT_1m_Un_pvalue,file="results/DEG_datasets/DESeq_DEG_Kbk_vs_WT_1m_Untreated_pvalue_cutoff_0.05.csv")
...
sig_genes_pvalue<-Kbk_vs_WT_1m_Un_pvalue$gene
...
Kbk_vs_WT_1m_Un_padj<-res_tbl %>% filter(padj<0.05)
Kbk_vs_WT_1m_Un_padj


genes_of_interest<-rownames(Kbk_vs_WT_1m_Un_padj)
genes_of_interest
...
png("figures/Heatmap_Kbk_vs_WT_1m_Untreated.png",width=500,height=500)
