...
# Kabuki DMSO_PEG vs DCF DEG and heatmaps --------------------------------------
...
MG_Kbk_DMSO_PEG_DCF<-subset(MG,Age=="3m" & Treatment %in% c("DMSO_PEG","DCF" ) & Genotype =="Kbk")

#reassign to another variable: seurat
seurat<-MG_Kbk_DMSO_PEG_DCF
...
write.csv(metadata,file="results/metadata_Kbk_DMSO_PEG_vs_DCF.csv")
...
# Analysis: The batch effect is removed(MG_Kbk_TCF_DCF) -------------------
...
#setwd("C:/Users/kppra/OneDrive/Desktop/scRNAseq_Seurat5/data_seurat5")
saveRDS(dds,"data/dds_obj_Kbk_3m_DMSO_PEG_vs_DCF.rds")
...
Kbk.3m.DMSO_PEG<-levels(factor(metadata$group))[2]#"Kbk.3m.TCF"
Kbk.3m.DCF<-levels(factor(metadata$group))[1]#"Kbk.3m.DCF"


contrast <- c("group", Kbk.3m.DCF, Kbk.3m.DMSO_PEG)
library(DESeq2)
...
Kbk_DCF_vs_DMSO_PEG_pvalue<-res_tbl %>% filter(pvalue<0.05)


#setwd("C:/Users/kppra/OneDrive/Desktop/scRNAseq_Seurat5/results_seurat5")
write.csv(Kbk_DCF_vs_DMSO_PEG_pvalue,file="results/DEG_datasets/DESeq_DEG_Kbk_DCF_vs_DMSO_PEG_pvalue_cutoff_0.05.csv")
...
sig_genes_pvalue<-Kbk_DCF_vs_DMSO_PEG_pvalue$gene
...
Kbk_DCF_vs_DMSO_PEG_padj<-res_tbl %>% filter(padj<0.05)
Kbk_DCF_vs_DMSO_PEG_padj


genes_of_interest<-rownames(Kbk_DCF_vs_DMSO_PEG_padj)
genes_of_interest
...
png("figures/Heatmap_Kbk_DMSO_PEG_vs_DCF_treatment.png",width=500,height=500)
ht <- Heatmap(sig_counts_s, 
              ...
              column_split = factor(rep(c("PEG.F",
                                          "PEG.M",
                                          "HPBCD.F",
                                          "HPBCD.M"), each = 3),
                                    levels=c("PEG.F",
                                             "PEG.M",
                                             "HPBCD.F",
                                             "HPBCD.M")),  # Split columns into four groups
