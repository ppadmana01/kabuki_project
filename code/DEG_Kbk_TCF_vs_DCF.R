...
# Kabuki TCF vs DCF DEG and heatmaps --------------------------------------
...
MG_Kbk_DCF_TCF<-subset(MG,Age=="3m" & Treatment %in% c("TCF","DCF" ) & Genotype =="Kbk")

#reassign to another variable: seurat
seurat<-MG_Kbk_DCF_TCF
...
write.csv(metadata,file="results/metadata_Kbk_TCF_vs_DCF.csv")
...
#setwd("C:/Users/kppra/OneDrive/Desktop/scRNAseq_Seurat5/data_seurat5")
saveRDS(dds,"data/dds_obj_Kbk_3m_TCF_vs_DCF.rds")
...
Kbk.3m.TCF<-levels(factor(metadata$group))[2]#"Kbk.3m.TCF"
Kbk.3m.DCF<-levels(factor(metadata$group))[1]#"Kbk.3m.DCF"


contrast <- c("group", Kbk.3m.TCF, Kbk.3m.DCF)
library(DESeq2)
...
Kbk_TCF_vs_DCF_pvalue<-res_tbl %>% filter(pvalue<0.05)



write.csv(Kbk_TCF_vs_DCF_pvalue,file="results/DEG_datasets/DESeq_DEG_Kbk_TCF_vs_DCF_pvalue_cutoff_0.05.csv")
...
sig_genes_pvalue<-Kbk_TCF_vs_DCF_pvalue$gene
...
Kbk_TCF_vs_DCF_padj<-res_tbl %>% filter(padj<0.05)

genes_of_interest<-rownames(Kbk_TCF_vs_DCF_padj)
genes_of_interest
...
png("figures/Heatmap_Kbk_TCF_vs_DCF_treatment.png",width=500,height=500)
